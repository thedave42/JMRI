...
| 9  | Throttle / Dynamic Brake    | Continuous analog, bipolar                                  | byte 1 — full Throttle `0x3a`, full Dynamic Brake `0xdd`                                                           | Down = throttle (centre→max). Centre = idle. Up = dynamic brake (centre→max). The Idle byte value is intentionally not captured (calibrated at runtime). |
...
| 1  | Throttle / Dynamic Brake (analog: `0x3a` full Throttle .. `0xdd` full Dynamic Brake) |
