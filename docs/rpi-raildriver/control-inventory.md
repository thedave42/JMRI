...
| 11 | Independent Brake           | Continuous analog plus a momentary bail-off switch          | byte 3 (primary axis) — full release `0xc0`, full application `0x41`; byte 4 (secondary) — at-rest reference `~0x96`, asserted (bail-off pressed) `~0xd0` (pressure-graded; observed asserted values span `~0xc0..0xda`) | The bail-off is mechanically a single momentary switch ("press the lever right") that activates regardless of byte-3 position. The reported byte-4 value is pressure-graded, so the JMRI mapping treats it as binary against a midpoint threshold between the Rest and Pressed calibration values. |
...
| 4  | Independent Brake bail-off switch (momentary; byte 4 ~`0x96` at rest, ~`0xd0` when pressed; pressure-graded asserted range ~`0xc0..0xda`) |
