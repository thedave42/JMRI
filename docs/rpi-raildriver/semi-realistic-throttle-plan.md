...
The integration body uses `F_drive = min(TE_avail, P/v_guard)` (with `P` and `TE` scaled by `driverPowerPct` and the steam-power exponent), `F_rr = c_rr · m_total · g`, plus the brake-force terms added in stages 3–5. Mechanical-transmission locos coast at 15/27/41 mph crossings (3.5 s gear pause). The integration runs symmetrically (no `a < 0` clamp) so accel and decel are the same code path with different forces active.
