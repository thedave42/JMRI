...
public final class SemiRealisticThrottleEngine {
    ...
    private volatile float brakeMaxDecel;         // wall-clock m/s² decel @ 100% indep brake. Default 4.0.
    private volatile float airBrakeMaxDecel;      // wall-clock m/s² decel @ 100% air line.    Default 6.0.
    private volatile float dynBrakeMaxDecel;      // wall-clock m/s² decel @ 100% dyn brake (peak, before taper). Default 1.6.
...
| Mech brake 80 mph → 0 (`BRAKE_MAX_DECEL = 4.0`) | operator-controlled | `v / decel` | ~9 s | ~9 s |
| Air brake EMG 80 mph → 0 (`AIR_BRAKE_MAX_DECEL = 6.0`) | operator-controlled | `v / decel` | ~6 s | ~6 s |
| Dyn brake full 60 mph → 5 mph (`DYN_BRAKE_MAX_DECEL = 1.6`) | operator-controlled | `(v − v_min) / decel` | ~15 s | ~15 s |
...
|---|---:|---|---|
| `BRAKE_MAX_DECEL` | 4.0 m/s² wall-clock | operator-controlled | Indep brake at 100 % decel rate; default tuned so brakes dominate scale-aware coast at typical layout scales |
| `AIR_BRAKE_MAX_DECEL` | 6.0 m/s² wall-clock | operator-controlled | Auto brake at EMG decel rate |
| `DYN_BRAKE_MAX_DECEL` | 1.6 m/s² wall-clock | operator-controlled | Dyn brake peak (before speed taper), loco mass only |
...
<raildriver-calibration version="2">
    ...
    <semiRealistic>
        ...
        <brakeMaxDecel>4.0</brakeMaxDecel>
        <airBrakeMaxDecel>6.0</airBrakeMaxDecel>
        <dynBrakeMaxDecel>1.6</dynBrakeMaxDecel>
...
4. Mode ON, indep brake at Full Application + throttle at zero, Light-engine defaults: loco decelerates at **4.0 m/s² wall-clock** (= `BRAKE_MAX_DECEL`). Time from 80 mph (35.8 m/s prototype) to 0 ≈ **9 s wall-clock at any layout scale** (operator-controlled brake; not scale-compressed).
5. Mode ON, indep brake mid-travel while throttle is at full: integrator computes net force in wall-clock m/s². With Light-engine defaults at low speed, F_drive ≈ 350 kN, F_brake_mech at 50 % = 260 kN. Net force = +90 kN, loco continues to accelerate slowly at ~0.69 m/s² wall-clock. At higher speeds where `F_drive = P/v` drops below `F_brake_mech`, loco settles at ~19 mph equilibrium (P_avail = 2200 kW ÷ 260 kN). **Emergent behaviour, not coded as a special case.**
...
2. Auto brake at EMG, throttle at zero, Light-engine: loco decelerates at **6.0 m/s² wall-clock** (= `AIR_BRAKE_MAX_DECEL`). 80 mph → 0 in **~6 s wall-clock at any layout scale** (operator-controlled, scale-invariant).
3. Auto brake at 50 % + indep brake at Full Release: `F_brake_air = 0.5 · 6.0 · m_total`. Combined with `F_brake_mech = 0`, net decel ≈ 3.0 m/s² wall-clock. When indep brake is also applied at 50 %: `F_brake_total = 0.5·6.0·m + 0.5·4.0·m = 5.0·m`, decel ≈ 5.0 m/s² wall-clock — **forces sum, no `min(...)` clipping. Stronger braking than either alone, which is the prototype reality**: the EngineDriver `min(airLineAsBrakePcnt, brakePcnt)` rule [research §3.4] was an arcade simplification we deliberately don't reproduce.
...
2. Lever at full DYN BRAKE, Light-engine, throttle and brakes off, at 60 mph: `F_brake_dyn = 1.0 · 1.6 · 130 000 · 1.0 = 208 kN`, decel **1.6 m/s² wall-clock** (operator-controlled, scale-invariant). LED shows `DBr`.
3. Same as #2 but at 3 mph prototype velocity (below taper threshold): `speedTaper(3 mph) = min(1, 1.34/2.24) = 0.6`, decel ≈ 0.96 m/s² wall-clock. Below ~0.5 mph dyn brake contributes essentially nothing.
4. Lever at full DYN BRAKE + Auto brake at EMG simultaneously, Light-engine at 60 mph: `F_brake_dyn = 208 kN`, `F_brake_air = 1.0 · 6.0 · 130 000 = 780 kN`, total = 988 kN, decel ≈ 7.6 m/s² wall-clock — emergent from force summation, no special-case code. Heavier (e.g. Unit train) consist scales `F_brake_air` up but not `F_brake_dyn`, so dyn brake's relative contribution is much smaller — prototypically correct (loco-only dyn brake on a heavy train is not the primary deceleration mechanism).
...
- **Brake constants (decided 2026-05-02; defaults retuned 2026-05-03): scenario-independent, operator-controlled, stated in wall-clock m/s² perceived rate.** `BRAKE_MAX_DECEL = 4.0 m/s² wall-clock`, `AIR_BRAKE_MAX_DECEL = 6.0 m/s² wall-clock`, `DYN_BRAKE_MAX_DECEL = 1.6 m/s² wall-clock` (peak, before speed taper), `DYN_BRAKE_V_MIN = 5 mph prototype` (velocity threshold; not wall-clock, not scale-compressed), `c_rr = 0.002` (dimensionless prototype-physics coefficient). All exposed as Settings-tab fields. Brake forces are `decelRate · m_total` (mechanical and air, scaling with consist mass) or `decelRate · m_loco · speedTaper(v)` (dyn brake, loco-only and tapered) — so heavier consist gets the same brake decel rate but smaller dyn-brake contribution, prototypically correct. Brake decel rates are scale-invariant by design — same wall-clock time-to-stop at HO, N, or any other scale. The defaults were retuned from the prototype-tradition values (1.0 / 1.5 / 0.4) so brakes meaningfully dominate the scale-aware coast term: at N (`physicsTimeScale = 160`) coast decel is ~3.14 m/s², so 4.0 mech wins by ~25 % standalone and a stacked mech + air gives a ~3 s 80 mph → 0 wall-clock stop. Operators wanting the prototype tradition values can re-enter them on the Settings tab.
...
- **Settings-field validation ranges (decided 2026-05-02; brake-decel ranges widened 2026-05-03 to fit the retuned defaults):** loco mass 1–500 t; loco power 1–10 000 kW; loco TE 1–2000 kN; additional consist mass 0–50 000 t; driver power 0–100 %; rolling resistance coefficient 0.0001–0.05; mechanical brake max decel 0.1–20.0 m/s² wall-clock; air brake max decel 0.1–20.0 m/s² wall-clock; dynamic brake max decel 0.0–10.0 m/s² wall-clock; dynamic brake taper threshold 0–20 mph prototype; physics time scale 0.1–1000 (or `auto`); ESU thresholds 1–100 monotonically increasing; ESU function numbers 0–28. Validation runs on Save and Apply per §2.4's flow; failures auto-select the offending tab and leave dirty set.
