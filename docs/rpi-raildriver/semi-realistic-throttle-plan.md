...
### In scope (split into stages 1–6, each independently shippable)
...
**Stage 2 — Physics integration engine + bypass switch + scenario picker + independent brake + toolbar mode toggle.** First stage where the operator can opt in and run the loco under physics. Adds the `SemiRealisticThrottleEngine` class (50 ms tick on a worker thread, full integration body including the `F_brake_mech` term so the operator has a working brake from day one), the `LoadScenario` enum and Settings-tab picker (all 6 scenarios incl. Custom), the persisted-vs-live `enabled` split (§2.3), the toolbar Jynstrument (§2.6), and the `hidDeviceAttached` re-enable for hot-plug PCS firing. Physics conventions: mass/power/TE/rolling-resistance forces, steam-power exponent 0.85, mechanical-transmission gear-change pause at 15/27/41 mph, `getPhysicsMaxSpeedKmh()` cap as hard ceiling on `v` and `v_target`. Continuously-scheduled tick (no `a < 0` clamp) so the same code runs accelerating or decelerating; brake-force terms `F_brake_air` and `F_brake_dyn` are present in the integration equation but their input fields stay zero until stages 4–5. When semi-realistic mode is OFF (default), behaviour is identical to stage 1's bypass path. When ON, the throttle lever (Axis 1 above Idle High) sets `v_target`, the calibrated indep-brake position (Axis 3) drives `F_brake_mech = (brakePct/100) · BRAKE_MAX_DECEL · m_total`, and the integrator walks `v` toward target under the resulting net force. Defaults match a single-loco "Light engine" out of the box (see §2.4.1) so users with empty rosters get sensible behaviour with zero configuration.
...
- **Deeper integration parameters configurable via UI.** The Settings tab exposes `BRAKE_MAX_DECEL`, `AIR_BRAKE_MAX_DECEL`, `DYN_BRAKE_MAX_DECEL`, `DYN_BRAKE_V_MIN`, `ROLLING_RESISTANCE_COEFF`, and per-scenario `additionalWeightTonnes` / `driverPowerPercent` / loco mass / power / TE overrides as user-editable fields (in scope). Lower-level integration parameters (50 ms slice time, gear-pause thresholds 15/27/41 mph and 3.5 s coast duration) stay hardcoded (out of scope).
...
#### Implementation: Save flow and session-only toggle

The Settings-tab Save/Apply path and the Jynstrument toolbar click are implemented as two distinct mechanisms with different persistence semantics:

**Settings tab Save/Apply** (in `RailDriverSettingsFrame.doSaveOrApply`):

1. Build a fresh `RailDriverCalibration working` instance.
2. Call `settingsTab.validateAndApplyTo(working)` — copies in-window values into `working.semiRealistic()`, including `working.semiRealistic.persistedEnabled = checkbox.isSelected()` and `working.semiRealistic.liveEnabled = working.semiRealistic.persistedEnabled` (the §2.3 rule that Save sets `liveEnabled := persistedEnabled`).
3. Call `calibrationTab.validateAndApplyTo(working)` — copies calibration values via `working.copyCalibrationFieldsFrom` so it does NOT clobber the `<semiRealistic>` subtree the Settings tab just wrote.
4. `working.save(file)` writes XML.
5. `RailDriverMenuItem.reloadCalibration()` snapshots the OLD `persistedEnabled`/`liveEnabled` from the in-memory calibration *before* replacing it, reloads from disk, pushes new settings to the engine via `engine.updateSettings(s)` + `engine.setLiveEnabled(s.liveEnabled)` (which performs the §2.3 OFF↔ON handover when the live value moved), and fires `"persistedEnabledChanged"` / `"liveEnabledChanged"` PCS events for the values that changed.

There is no `applyPersistedEnabled` entry-point on `RailDriverMenuItem` — earlier drafts of this plan listed one, but the Settings tab needs to persist the entire calibration (including the calibration tab's per-axis bytes), not just the `persistedEnabled` flag, so a single-flag entry point would only complicate the flow. The Save path uses `working.save(file)` + `reloadCalibration()` directly.

**Jynstrument toolbar click** (in `RailDriverMenuItem.setSemiRealisticEnabledSessionOnly`):

```java
public void setSemiRealisticEnabledSessionOnly(boolean enabled);
```

1. Snapshot `wasLive = settings.liveEnabled` (BEFORE mutation).
2. No-op early return when `wasLive == enabled`.
3. Mutate `settings.liveEnabled = enabled` in memory only — does NOT touch `persistedEnabled`, does NOT write XML.
4. Call `engine.setLiveEnabled(enabled)` — the engine performs the §2.3 OFF↔ON handover (read `throttle.getSpeedSetting()` on the EDT, seed `v_fs`, start ticking; or pause ticking on ON→OFF).
5. Fire `"liveEnabledChanged"` so the Jynstrument icon and any other live-state observers update. The Settings tab does NOT subscribe to this event; its checkbox stays at `persistedEnabled` regardless.
...
- `"liveEnabledChanged"` — `newValue` = boolean. Fired by `setSemiRealisticEnabledSessionOnly` (Jynstrument click path) and by `reloadCalibration` after a Settings-tab Save when the live value changed across the reload. The engine listens to drive the integration tick on/off; the Jynstrument listens to update its icon. The Settings tab does **not** subscribe to this event — its checkbox is bound to `persistedEnabled` and pending-edit state.
- `"persistedEnabledChanged"` — `newValue` = boolean. Fired by `reloadCalibration` after a Settings-tab Save when the persisted value changed across the reload. The Settings tab subscribes to refresh its checkbox display when the persisted value changes via a code path other than the in-window edit (e.g. a `reloadCalibration()` triggered externally, or a Save initiated from another window).
...
### 3.2 Stage 2 — Physics engine + bypass + scenario picker + independent brake + toolbar mode toggle

**Goal:** first stage where `liveEnabled = ON` produces a usable feature. The throttle lever sets `vTarget_fs` and the integrator walks `v_fs` toward it under loco physics; the calibrated independent-brake lever (Axis 3) provides a working `F_brake_mech` term so the operator can stop. The scenario picker is wired so the operator can change consist mass / driver power. The toolbar Jynstrument is auto-installed for fast session-level mode toggling.

The integration body uses `F_drive = min(TE_avail, P/v_guard)` (with `P` and `TE` scaled by `driverPowerPct` and the steam-power exponent), `F_rr = c_rr · m_total · g`, and `F_brake_mech = indepBrakeFraction · brakeMaxDecel · m_total`. The remaining brake terms `F_brake_air` and `F_brake_dyn` are present in the equation as zero-valued summands until stages 4–5. Mechanical-transmission locos coast at 15/27/41 mph crossings (3.5 s gear pause). The integration runs symmetrically (no `a < 0` clamp) so accel and decel are the same code path with different forces active.
...
*New:*
- `java/src/jmri/util/usb/SemiRealisticThrottleEngine.java` — engine with the integration tick / `ScheduledExecutorService` / DccThrottle access. Throttle and indep-brake paths fully wired; air-brake / dyn-brake input fields stay zero until stages 4–5. Worker thread does the math; EDT does the `setSpeedSetting`.
- `java/src/jmri/util/usb/SemiRealisticSettings.java` — settings POJO with load + save methods, mirroring `RailDriverCalibration`'s structure. Holds the physics constants (`brakeMaxDecel`, `airBrakeMaxDecel`, `dynBrakeMaxDecel`, `dynBrakeVMinMph`, `rollingResistanceCoeff`), per-scenario defaults table, the `persistedEnabled` and `liveEnabled` fields, and (placeholder, used by stage 6) the `decoderBrakeMode` enum.
...
- `java/src/jmri/util/usb/SemiRealisticSettingsPanel.java` — the Settings tab content. Replaces stage 1's placeholder. Implements the `isDirty / addDirtyChangeListener / validateAndApplyTo / resetToFile` contract from §2.4. Disables fields based on the `Enable semi-realistic mode` checkbox; switches loco-mass/power/TE rows between read-only "auto" display and editable when scenario = Custom. Decoder-brake-mode dropdown is present but has only `None` available (the `ESU` option is enabled in stage 6).
...
- `java/src/jmri/util/usb/SemiRealisticSettingsPanel.java` (already New above; called out here for the avoidance of doubt: stage 1 ships a placeholder, stage 2 fills it in).
- `java/src/jmri/util/usb/RailDriverMenuItem.java`:
...
- `java/src/jmri/util/usb/Bundle.properties` — no changes in stage 2. Settings-tab labels (scenario names, physics field labels, brake constant labels, button labels, status messages) are kept as hardcoded English literals in `SemiRealisticSettingsPanel.java`, matching the existing pattern in `CalibrationTabPanel.java` (which inherited hardcoded English from the predecessor `RailDriverCalibrationFrame`). Externalising both panels' labels to Bundle keys is deferred to a future i18n pass that covers the whole RailDriver UI consistently — see §8 future work.

**Acceptance:**
1. The Settings tab is now functional: scenario picker shows all 6 scenarios; selecting `Custom` enables loco-mass/power/TE/consist/driver-power fields; physics-constant fields (mechanical brake decel, rolling resistance, etc.) are editable and validated per §6's validation ranges.
2. Mode OFF: throttle behaves exactly as in stage 1 (operator-perceptibly identical).
3. Mode ON, default Light-engine scenario, empty roster (no physics fields populated), indep brake at Full Release: moving the throttle lever from idle to full speed produces a visible ramp on the loco. Time from 0 to top speed is on the order of 30–60 s prototype, dominated by power-limited integration above ~14 mph. **No snap, no jitter, no missed slots** during a 5-minute lever-sweep session.
4. Mode ON, indep brake at Full Application + throttle at zero, Light-engine defaults: loco decelerates at ≈ 1.0 m/s² (= `BRAKE_MAX_DECEL`). Time from 80 mph (35.8 m/s prototype) to 0 ≈ 36 s.
5. Mode ON, indep brake mid-travel while throttle is at full: integrator computes net force. With Light-engine defaults at low speed, `F_drive ≈ TE_avail = 350 kN`, `F_brake_mech` at 50 % = `0.5 · 1.0 · 130 000 = 65 kN`. Net force = +285 kN, loco continues to accelerate at ~2.2 m/s². At higher speeds where `F_drive = P/v` drops below `F_brake_mech`, loco settles at the equilibrium speed. **Emergent behaviour, not coded as a special case.**
...
11. The toolbar mode-toggle Jynstrument auto-installs on first `Debug → RailDriver Throttle (built in)` click. The icon shows the current **live** `enabled` state. Clicking it flips the live state **for this session only** (no XML write). The Settings tab's `Enable semi-realistic mode` checkbox shows the **persisted** value (last-saved or default), independent of any session-only Jynstrument toggles. Right-clicking the Jynstrument shows a `Settings...` item that opens the unified Settings frame. To make a session-level Jynstrument change persist, the operator opens the Settings window, sets the checkbox to match, and Saves.
12. The Jynstrument is idempotent on repeat attaches within a session — opening the throttle, closing it, and re-opening via the Debug menu does NOT add a second copy of the toggle to the toolbar.
13. If the operator explicitly removes the Jynstrument from the toolbar (right-click → Quit), it stays removed for the rest of the throttle window's lifetime. Re-clicking `Debug → RailDriver Throttle (built in)` triggers `attachThrottleWindow()` again, which **does** re-add the toggle — this is treated as a deliberate operator action equivalent to a fresh attach.
14. RailDriver hot-plug: unplugging the device fires `"railDriverConnected"` (false), greying the Jynstrument; replugging fires `"railDriverConnected"` (true), un-greying it. The Debug-menu lifecycle is unaffected.
15. Pre-existing XML files (schema `version="2"` from stage 1, no `<semiRealistic>` subtree) load cleanly — semi-realistic fields populate from defaults (disabled, Light engine); saving produces a `version="2"` file with the new `<semiRealistic>` subtree.
...
- **Engine architecture (decided 2026-05-02): symmetric physics integration on a 50 ms tick.** A single integration loop computes net force `a = (F_drive − F_rr − F_brake_mech − F_brake_air − F_brake_dyn) / m_total` on a 50 ms tick; the same loop runs accelerating or decelerating with the sign of `a` falling out of which forces are active. Throttle, indep brake, auto brake, and dyn brake each contribute their own force term — no regime detection, no "throttle fighting brake" branch, no separate accel/decel paths. `getPhysicsMaxSpeedKmh()` clamps both `vTarget` and integrated `v_fs`. See §2.1 for the engine class skeleton and §2.2 for the threading contract.
