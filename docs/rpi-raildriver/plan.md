...
5. **The 14-byte HID input report layout is empirically validated** through 5 independent capture runs analysed by `rd-analyze.sh`. The canonical bit-for-bit physical-control mapping lives in `docs/rpi-raildriver/control-inventory.md`; the parser-relevant split is:

   - **Bytes 0..6** — 7 analog axes (8-bit unsigned), one axis per byte: reverser, throttle/dynamic-brake, auto brake, independent brake primary, independent brake secondary/bail-off, wiper, lights.
   - **Bytes 7..13** — packed button bits, parsed by `RailDriverMenuItem.java:201–232` as 56 single-bit "buttons" via `n = 8*(i-7) + bit`. Of those 56 parser slots, **44 are driven by physical controls** (front-edge button grid, SPDT #42, hat #43, Range, E-Stop, Alert, Sand, P, Bell, Horn — see `control-inventory.md` for which bit drives which control), and **12 are firmware-unused**: byte 12 bits `0x10..0x80` (parser slots 44..47) and byte 13 (parser slots 48..55, invariant at `0x35`).

   This matches the parser in `RailDriverMenuItem.java` (lines 191–232) byte-for-byte. The kernel HID descriptor declares 56 buttons (`Usage Min = Button 1`, `Usage Max = Button 56`, packed as 56 × 1 bit; verified by `xxd /sys/class/hidraw/hidraw0/device/report_descriptor`) and `KEY=ffffffffffffff` in `/proc/bus/input/devices` — exactly matching the parser's 56-position decoding. The 12 firmware-unused bits are declared as buttons 44..55 in the descriptor but the firmware on this controller never drives them (likely because the Modern Desktop has fewer than 56 distinct buttons in hardware, and PI Engineering uses a common report layout across the RailDriver product family).

   **Firmware-unused bits confirmed.** All 5 earlier captured logs (2,046 reports) plus the focused capture procedure in `docs/rpi-raildriver/byte13-capture-test.md` (2,323 reports under exhaustive sweep of every analog lever and every physical control on the unit) show byte 13 invariant at `0x35` and byte 12's high nibble (`0x10..0x80`) invariant at `0`. The parser's mapping for bits 44..55 is dead code in practice — the diff loop produces no events when the bytes never change — but it remains in place so that any future firmware revision that starts populating these bits will surface immediately. See the §4.5 #1 firmware-unused-bits pin-down test for the regression guard.
...
- The button-to-function mapping baked into the Java code (see `RailDriverMenuItem.propertyChange`, `RailDriverMenuItem.java:482–765`). Physical-control labels are from `docs/rpi-raildriver/control-inventory.md`; the source-comment names in parentheses come from the MSTS-keymap heritage and do not always match the device's actual labels:
   - **Parser slot 0** (front-edge button, user-assignable) → **silently dropped.** `RailDriverMenuItem.java:744` filters with `fNum > 0`, so the bit corresponding to slot 0 never toggles any function. This is an unambiguous oversight — F0 is a real DCC function (typically headlight) and one of the 28 user-assignable front-edge buttons should be available to toggle it. **Not fixed in this branch** — flagged as a follow-up issue.
   - **Parser slots 1..27** (the remaining 27 of the 28 user-assignable front-edge buttons) → F1..F27 by parser slot number.
   - **Parser slots 28/29** (SPDT #42 up/down — source comment: "zoom/rocker") → `AddressPanel.selectRosterEntry()` / `AddressPanel.dispatchAddress()` (lines 624–639).
   - **Parser slots 30..33** (hat #43 up/right/down/left — source comment: "four way panning") → select previous roster entry / `nextThrottleFrame()` / select next roster entry / `previousThrottleFrame()` respectively (lines 640–688).
   - **Parser slots 34/35** (Range #1 up/down — source comment: "Gear Shift Up/Down", `shuntFn`) → `setFunction(3, false)` / `setFunction(3, true)` — i.e. the SPDT direction sets F3 OFF/ON. The Java source's "Gear Shift" naming pre-dates this hardware revision; the device label is **Range**.
   - **Parser slots 36/37** (E-Stop #2 up/down — source comment: "Emergency Brake") → `throttle.setSpeedSetting(-1)` (lines 703–709). Both directions of the SPDT trigger emergency stop.
   - **Parser slot 38** (Alert #3 — source comment: "Alerter", `alertFn`) → F6.
   - **Parser slot 39** (Sand #4 — source comment: "Sander", `sandFn`) → F7.
   - **Parser slot 40** (P #5 — source comment: "Pantograph", `pantoFn`) → F8. The device label is just "P"; the "Pantograph" interpretation is the Java code's assumption (also recorded as an assumption in `control-inventory.md`).
   - **Parser slot 41** (Bell #6, `bellFn`) → **F1** — not F3 as the script-based docs say.
   - **Parser slots 42/43** (Horn #7 up/down, `hornFn`) → F2.
...
0. **Firmware-unused-bits capture test (hardware required, run once before any code change).** *Resolved 2026-05-01 — see the result block at the bottom of `docs/rpi-raildriver/byte13-capture-test.md`.* Byte 13 is empirically a constant sentinel (`0x35`) and byte 12's high nibble (`0x10..0x80`) is empirically constant `0` across 2,323 reports under exhaustive sweep of every physical control on the unit; the device's HID descriptor declares those positions as buttons 44..55 but the firmware never sets them. The parser is correct as-is; the §4.5 #1 firmware-unused-bits sub-bullets stay in place. The §6 risk row has been updated to record the resolved interpretation. The capture procedure itself is left in `byte13-capture-test.md` for use against any future firmware revision or different RailDriver SKU.
...
1. **Byte-parser test (no hardware required).** The per-report decode loop currently lives inline inside the polling thread (`RailDriverMenuItem.java:201–232`); to be testable it needs to be reachable from a test as a pure function of two byte buffers. Inject a sequence of recorded 14‑byte reports (5 captures totalling 2,046 reports / 680 unique payloads, checked in under `docs/rpi-raildriver/test-data/hidraw-raildriver*.log` as `xxd -c 14` formatted text) into that decode entry point, and assert that:
   ...
   - **For the committed fixtures and for the device's current firmware**, parser slots 44..55 produce no events — every report has byte 12 high nibble (`0x10..0x80`) clear and `byte13 == 0x35`. This is the production contract on the parser today (see §4.5 #0 / `byte13-capture-test.md` for the empirical confirmation). The pin-down test below complements this assertion as a regression detector.
   - **Synthetic-input parser-behaviour pin-down test.** Hand-construct two reports where only the firmware-unused bits differ — e.g. byte 13 `0x35` → `0x36` (XOR `0x03`, bits 0 and 1 flipped) **and** byte 12 high nibble `0x00` → `0x30` (XOR `0x30`, bits 4 and 5 flipped) — and assert that the parser emits exactly four events with names `"44"`, `"45"`, `"48"`, and `"49"`. This locks in the parser's behaviour: byte 12 high nibble is parser slots 44..47 and byte 13 is parser slots 48..55, both via the `i >= 7` else-branch at `RailDriverMenuItem.java:216`. The assertion is a **regression detector**: if a future firmware revision (or a different RailDriver SKU) starts populating these bits, the parser will emit the events and downstream code will see them; this test ensures that pathway is exercised in CI even though no committed fixture exercises it. **Do not invert this assertion.**
...
- Both English help pages should cross-link to `docs/rpi-raildriver/control-inventory.md` for users who want the bit-for-bit HID layout and physical-control reference.
...
1. On a clean Pi 4 (64‑bit Pi OS) with this branch built and installed via `build-and-install-to-jmri.sh`, after running the udev steps:
   ...
   - **Per-axis physical-control verification.** Sweep each lever in isolation and confirm the corresponding `firePropertyChange Axis N` line in the log matches the empirically validated mapping in `docs/rpi-raildriver/control-inventory.md` (reverser → Axis 0, throttle/dyn-brake → Axis 1, auto-brake → Axis 2, indep brake primary → Axis 3, indep brake secondary/bail-off → Axis 4, wiper rotary → Axis 5, lights rotary → Axis 6). This is an end-to-end regression check on the JMRI menu wiring against the inventory; the inventory itself was empirically validated via 5 independent capture runs analysed by `rd-analyze.sh`, so a mismatch here would indicate a JMRI-side regression rather than an unknown layout. If any axis is misnumbered, downstream throttle behaviour will be wrong; this branch's response is to document the mismatch and update `RailDriverMenuItem.propertyChange` (or, if the divergence is too deep, to defer the fix to a follow-up issue and ship the branch with the documentation correction only).
...
| **Firmware-unused bits: confirmed constant.** Resolved 2026-05-01 via `docs/rpi-raildriver/byte13-capture-test.md`: byte 13 = `0x35` and byte 12 high nibble (`0x10..0x80`) = `0` invariant across 2,323 reports under exhaustive sweep of every analog lever and every physical control on the unit (bytes 0..11 plus byte 12's low nibble all showed activity). Per the device's own HID descriptor (`Usage Min = Button 1`, `Usage Max = Button 56`, packed as 56 × 1 bit), those positions are declared as buttons 44..55 but the firmware never sets them. The parser's mapping for bits 44..55 is dead code in practice — the diff loop produces no events when the bytes never change. | Resolved | None | None — the §4.5 firmware-unused-bits pin-down test stays in place to flag any future firmware revision that starts populating these bits. |
...
- `docs/rpi-raildriver/control-inventory.md` — canonical bit-for-bit physical-control / HID-layout reference (empirically validated via the captures below)
