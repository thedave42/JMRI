...
- **Fixing the device-detach NPE in `RailDriverMenuItem.hidDeviceDetached`.** The handler nulls `hidDevice` but does not interrupt the polling thread, which then NPEs on the next `hidDevice.isOpen()` call. Pre-existing latent bug; fixing it is a follow-up so this branch's diff stays focused on the Pi/aarch64 enablement.
- **Multiple RailDrivers attached at once.** `hidServices.getHidDevice(VENDOR_ID, PRODUCT_ID, null)` returns whichever device the OS lists first. Two-controller setups are unsupported in this branch.
...
- The button-to-function mapping baked into the Java code differs from the Jython script's:
   ...
   - **Button 0 is silently dropped.** `RailDriverMenuItem.java:744` filters with `fNum > 0`, so the bit corresponding to button 0 never toggles F0. This appears to be a latent bug (or a deliberate "F0 reserved" choice that nobody documented). It is **not fixed in this branch** — flagged as a follow-up to investigate during the Commit 3 refactor (§5 Commit 3).
...
Add a new entry `RdBuiltIn` ("RailDriver Throttle (built in)") to the **Debug menu**, immediately following the existing `MenuRailDriverThrottle` script entry. The bundle key already exists in the base `Bundle.properties` and 4 of the 5 localized files (`Bundle_cs.properties`, `Bundle_de.properties`, `Bundle_fr.properties`, `Bundle_nl.properties`) — it was reserved for exactly this purpose and is currently dead code. **`Bundle_ca.properties` is missing the key**; commit 3 will either add it or accept that Catalan falls back to the English string from the base bundle.
...
1. **Byte-parser test (no hardware required).** Inject a sequence of recorded 14‑byte reports (5 captures totalling 2,046 reports / 680 unique payloads, checked in under `docs/rpi-raildriver/test-data/hidraw-raildriver*.log` as `xxd -c 14` formatted text) into a refactored protected method `processReportDelta(byte[] oldR, byte[] newR)`, and assert that:
   - changes in bytes 0–6 emit `firePropertyChange("Value", "Axis N", String value)`. Axis 1's numerical value lies in `[-1.0, +1.0]` per the `(2v − 1)` formula in `RailDriverMenuItem.java:208–211`; axes 0/2/3/4/5/6 lie in `(0.0, 1.0]`. The test asserts the numerical range only, **not** the physical-direction-to-value mapping (we have no synchronized photographic record of lever positions during capture).
   ...
   - **Synthetic-input parser-contract test:** hand-construct two reports where only byte 13 differs (e.g. `0x35` → `0x36`) and assert the parser emits no events. This catches the latent bug where the parser treats byte 13 as a button bitmap (`i >= 7`); a future firmware that varies byte 13 would otherwise surface phantom events 48..55 in production with no test coverage. If the parser is intentionally fixed to skip byte 13 in §5 Commit 3, this test still passes; if not, the test pins down the documented behaviour as "indices >= 48 are downstream-unhandled and should not fire."
...
2. **No-device behaviour.** Verify that selecting the menu item with no RailDriver attached logs an info message and does not throw.
3. **LED-encoding test (no hardware required).** Assert that `setLEDs(String)` produces the expected 7-byte report buffer for representative inputs: digits ("123"), letters ("Pro"), decimal-point handling ("8.8.8."), shorter strings, and unsupported characters. The seven-segment encoding is non-trivial (see `RailDriverMenuItem.java` lines 357–401 plus the `SevenSegment[]` and `SevenSegmentAlpha[]` tables), called every time `setupRailDriver` succeeds (line 133), and currently has zero coverage. Use a test seam that intercepts `sendMessage(byte[], byte)` so we don't need an open `HidDevice`.
4. **Default-throttle-layout fall-through smoke test.** `setupRailDriver` (lines 142–155) tries `LoadXmlThrottlesLayoutAction.loadThrottlesLayout`, throws/catches `IOException` when no default exists, then creates a fresh throttle window via `tfManager.createThrottleWindow()`. This path executes on day 1 of every new Pi install (no saved layout). Assert that, with no default-layout file present, the menu item brings up a throttle window without throwing. Hardware-independent (no actual `HidDevice` open required).
...
> Plan revision marker: `2026-04-29-r7`. When commit messages reference a `§5 Commit N`, they refer to this revision. If a future revision renumbers commits, the marker bumps so historical references can still be traced (see also §7 risk on plan/code drift).

### Commit 0 — Test-data fixtures (already merged on `rpi-raildriver` as `c21d79f5391`)
...
- Run the existing test (project is single-module Maven, single-module ant): `ant test-single -Dtest.includes=jmri/util/usb/RailDriverMenuItemTest.java` (or equivalently `mvn -B test -Dtest=RailDriverMenuItemTest`) on x86_64 Linux and aarch64.
...
- Extract the inner polling-loop body into a package-private `processReportDelta(byte[], byte[])` method that fires the same property changes.
- The polling thread becomes a thin wrapper that calls `processReportDelta` after each successful `hidDevice.read`.
- No behavioural change.

### Commit 4 — Tests
- Extend `RailDriverMenuItemTest.java` per §4.5: byte-parser tests against committed fixtures, synthetic byte-13 contract test, LED-encoding test, no-device behaviour, default-layout fall-through smoke test, and the existing constructor test.
...
- Likely needs a small test seam in `RailDriverMenuItem` (e.g. extracting `sendMessage` and the throttle-layout load to package-private overridable methods) so that the LED and default-layout tests can run without a real `HidDevice`. If so, the seam lands in §5 Commit 3 (refactor) and §5 Commit 4 just adds tests against it.
...
- No build wiring needed for the Linux dist: `build.xml`'s `package-linux` target already copies `${libdir}/linux/` recursively into `dist/Linux/JMRI/lib/linux/` (build.xml ~lines 2787–2789), so the new `udev/` subdirectory ships in the `.tgz` automatically.
...
- The launcher script (`PanelPro` at the line that builds `SYSLIBPATH`) discovers natives only by checking `lib/linux/<ARCH>/`, so an extra `udev/` sibling directory does not interfere with native-library discovery.
...
4. `ant test-single -Dtest.includes=jmri/util/usb/RailDriverMenuItemTest.java` is green (canonical JMRI invocation; the `mvn -B test -Dtest=RailDriverMenuItemTest` form must also pass).
...
| `hid4java-0.8.0`'s `linux-aarch64/libhidapi.so` is built against a glibc newer than Pi OS / Debian 12 ships, causing `UnsatisfiedLinkError` at JNA load time | Low | High | Smoke-test the new jar on a fresh Pi OS install (and on the oldest supported Debian release) before staging the binary in §5 Commit 1. Internal contingency: vendor the `.so` rebuilt locally against the older glibc, or fall back to system `libhidapi-hidraw0` (see rejected alternative in §4.2) |
...
| Pre-existing latent bug: `hidDeviceDetached` doesn't interrupt the polling thread; an unplug-during-use will NPE on next `hidDevice.isOpen()` | High (when a user unplugs) | Low (one stack trace, throttle window stops responding) | Documented as out-of-scope follow-up in §3; no in-branch fix |
...
| `RailDriverMenuItem.java` has an inactive code path treating byte 13 as buttons | Negligible | None | No spurious events because byte 13 is constant in observed firmware; document in plan, do not refactor in this branch |
