...
5. **The empirically validated 14-byte HID input report layout on a real RailDriver Modern Desktop is:**
   ...
   bytes 0..6   = 7 analog axes (8-bit unsigned)
                  ...
                  5: wiper rotary
                  6: lights rotary
   bytes 7..13  = 7 button-bitmap bytes (56 button positions)
   ```

   This matches the parser in `RailDriverMenuItem.java` (lines 191–232) byte-for-byte. The kernel HID descriptor declares 56 buttons (`KEY=ffffffffffffff` in `/proc/bus/input/devices`; `jstest` reports `56 buttons`), so the parser's 56-position decoding matches the device's declared button count exactly.

   **Caveat on byte 13.** Across all 5 captured logs (2,046 reports) byte 13 is invariant at `0x35`. Two interpretations are possible:
   - byte 13 is a constant sentinel/version byte and the parser's mapping of bits 48..55 to button events is dead code; or
   - byte 13 is part of the button bitmap and was never observed to change because the captures did not toggle the cab's latching switches (gear toggle, three-way reverser, headlight rotary, etc.) into a different position than they were in at the start.

   A live `jstest` snapshot of the same controller showed exactly 4 buttons in the "down" initial state (jstest button positions 16, 18, 20, 21), corresponding to physical latching switches the user had left engaged. `0x35 = 0b00110101` has exactly 4 bits set. Whether those bits live in byte 13 or in bytes 7..12 cannot be determined from the captures alone (the kernel HID-generic driver renumbers buttons and we do not have the device's HID descriptor). This question is resolved by the focused capture test in `docs/rpi-raildriver/byte13-capture-test.md` (a 5-minute hardware procedure that toggles each latching switch in turn while logging hidraw, then diffs byte 13). Run that test before committing the parser's behaviour either way.

   The plan below treats byte 13 as **part of the button bitmap** (the parser is correct, no "inaccuracy") because that is the safer assumption: if the test confirms byte 13 changes with the latching switches, no plan revision is needed; if the test confirms byte 13 is genuinely constant, only the §6 risk register row and the §4.5 byte-13 pin-down test need light edits, and the parser still does the right thing (no events emitted for an unchanging byte).
...
0. **Byte-13 capture test (hardware required, run once before any code change).** The plan's §1 #5 layout description treats byte 13 as part of the button bitmap, but the captured logs show it invariant at `0x35`. Resolve which interpretation is correct with the focused capture procedure in `docs/rpi-raildriver/byte13-capture-test.md`. Outcomes:
   - **Byte 13 changes when latching switches are toggled** → confirms the parser's existing mapping is correct as-is. The §4.5 #1 byte-13 sub-bullet below stays as written. The §6 risk row about byte 13 is removed (no risk to mitigate).
   - **Byte 13 stays at `0x35` regardless of any control state** → confirms it is a constant sentinel. The §4.5 #1 byte-13 sub-bullet stays (the parser's mapping for bits 48..55 is dead code but harmless). The §6 risk row is updated to "byte 13 confirmed sentinel via capture-test on `<date>`; no further action".
   - Either way, append the test result (one line + the `awk '{print substr($8,3,2)}' | sort -u` output) to `docs/rpi-raildriver/byte13-capture-test.md` so the question is settled in-tree.
...
1. **Byte-parser test (no hardware required).** The per-report decode loop currently lives inline inside the polling thread (`RailDriverMenuItem.java:201–232`); to be testable it needs to be reachable from a test as a pure function of two byte buffers. Inject a sequence of recorded 14‑byte reports (5 captures totalling 2,046 reports / 680 unique payloads, checked in under `docs/rpi-raildriver/test-data/hidraw-raildriver*.log` as `xxd -c 14` formatted text) into that decode entry point, and assert that:
   ...
   - **For the committed fixtures specifically**, byte 13 produces no events — every report has `byte13 == 0x35`. The test asserts this empirical property of the captures, not a contract on the parser. (Whether byte 13 is a true constant or a never-toggled latching-switch byte is settled by the capture test in #0 above; the parser's behaviour is the same either way.)
   - **Synthetic-input parser-behaviour pin-down test.** Hand-construct two reports where only byte 13 differs (e.g. `0x35` → `0x36`, XOR `0x03`, two bits flipped) and assert that the parser emits exactly two events with names `"48"` and `"49"`. This locks in the parser's current behaviour: byte 13 is treated as button bits 48..55 via the `i >= 7` else-branch at `RailDriverMenuItem.java:216`. **Do not plan to invert this assertion later.** If the capture test in #0 confirms byte 13 carries latching-switch state, this assertion is the production contract. If it confirms byte 13 is a constant sentinel, the assertion is still correct as a pin-down on the parser's actual code path (which would only run on a future firmware revision where byte 13 became live). Either way the assertion stays.
...
| **Byte 13 interpretation: constant sentinel vs. live latching-switch state.** All 5 captured logs (2,046 reports) show byte 13 invariant at `0x35`, but a live `jstest` snapshot of the same controller showed exactly 4 latching switches in the "down" state, and `0x35 = 0b00110101` has exactly 4 bits set. The bit-count alignment is suspicious. If byte 13 is in fact part of the button bitmap, the parser is correct as-is and no in-branch action is needed. If it is genuinely a constant sentinel, the parser's mapping for bits 48..55 is dead code (but harmless — the diff loop produces no events when the byte never changes). | Low (parser handles either interpretation correctly today) | Low (no spurious events under either interpretation) | Run the focused capture procedure in `docs/rpi-raildriver/byte13-capture-test.md` before merge. Append the result (one line + the `awk \| sort -u` output of byte-13 distinct values) to that file. Update this risk row to record the resolved interpretation. The §4.5 byte-13 pin-down test stays in place either way (see §4.5 #1 for why). |
